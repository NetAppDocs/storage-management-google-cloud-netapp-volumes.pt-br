...
NOTE: As alterações nas permissões do projeto do Google Cloud podem levar alguns minutos para entrar em vigor.
...
NOTE: Para ambientes de teste, use  `credentials-sa@wf-staging-netapp.iam.gserviceaccount.com` em vez da conta de serviço de produção. Substitua o valor do parâmetro  `--member` no comando acima por  `serviceAccount:credentials-sa@wf-staging-netapp.iam.gserviceaccount.com`.
